...
`status-line-with-tab-title.sh` is a wrapper that renders the status line *and* sets the iTerm2 tab title to `<repo>/<subpath> (claude) – <session name>`, where the session name is whatever you set with `/rename` (unnamed sessions show just `<repo>/<subpath> (claude)`). It delegates rendering to `status-line.sh` in the same directory, so keep the two files together.
