...
COST_DISPLAY=""
[ -n "$COST" ] && COST_DISPLAY=$(printf " ${GRAY}\$%.2f${RESET}" "$COST")
...
if [ -n "$PCT" ]; then
  ...
  echo -e "${CYAN}[$MODEL${RESET}${GRAY}/${EFFORT}${RESET}${CYAN}]${RESET}${COST_DISPLAY} ${DISPLAY_DIR}$BRANCH ${BAR} ~${FREE_PCT}% free"
else
  echo -e "${CYAN}[$MODEL${RESET}${GRAY}/${EFFORT}${RESET}${CYAN}]${RESET}${COST_DISPLAY} ${DISPLAY_DIR}$BRANCH"
fi
